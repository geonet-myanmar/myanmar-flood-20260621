...
PRE_FLOOD_DATE  = "2026-06-09"
POST_FLOOD_DATE = "2026-06-21"
...
# 100 m keeps each tile well under GEE's 48 MB single-request limit.
EXPORT_SCALE = 100

OUTPUT_HTML = "flood_inundation_map.html"
TEMP_DIR    = "_gee_tmp"      # deleted automatically after the map is built

# Max tile width/height in degrees for tiled GEE downloads.
# 1.5° tiles stay well under GEE's 48 MB per-request limit at 100 m.
TILE_DEG = 1.5
...
def download_as_geotiff(image: ee.Image, name: str,
    ...
    This function tiles the BBOX into TILE_DEG x TILE_DEG degree cells,
    downloads each tile separately, then merges them into one GeoTIFF
    with rasterio.merge -- no size limit, works at any resolution.
    """
...
def build_map(overlays: dict, flood_km2: float, output: str) -> None:
    ...
    ">
      ...
      <span style="color:#666;font-size:11px">Myanmar &mdash; Sagaing / Chin / Kachin</span>
